...
const MENU = { key: "report-marker", parentKey: "report", title: "Patok", url: "/admin/report/marker", icon: "Landmark", order: 7, isActive: true, isVisible: true };
const PERMS = {
...
try {
  ...
  console.log(existingMenu ? `Menu ${MENU.key}: SUDAH ADA — skip` : `Menu ${MENU.key}: BELUM ADA — akan dibuat (parent ${MENU.parentKey}, order ${MENU.order})`);
