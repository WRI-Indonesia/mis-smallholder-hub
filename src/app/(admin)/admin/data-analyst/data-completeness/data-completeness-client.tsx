...
export function DataCompletenessClient({ districts, initialFarmerGroups }: Props) {
  ...
  const handleDownload = async () => {
    ...
    const ringkasanRows = [
      ...
      { metrik: "Skor Profil Lembaga Petani", nilai: `${result.profileScore}%` },
    ...
    <div className="space-y-6">
                ...
                <PopoverTrigger
                  render={
                    <Button variant="outline" role="combobox" aria-expanded={groupComboOpen} className="w-[250px] justify-between h-9 font-normal text-left">
                      {selectedFarmerGroup === null ? <span className="text-muted-foreground">Pilih Lembaga Petani</span> : <span>{selectedGroupObj?.name}</span>}
                      <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                    </Button>
                  }
                />
      ...
      {!result ? (
        <Card className="flex flex-col items-center justify-center p-12 text-center border-dashed">
          ...
          <p className="text-muted-foreground font-medium">Pilih District dan Lembaga Petani, lalu klik Analisa</p>
        </Card>
      ) : (
            ...
            <CardContent className="pt-6">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <span className="inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-xs">
                  <ClipboardCheck className="h-3.5 w-3.5 text-muted-foreground" /> Profil Lembaga Petani <ScoreBadge score={result.profileScore} />
                </span>
...
function ProfileSection({ result }: { result: DataCompletenessResult }) {
  ...
  return (
    <SectionShell title="Profil Lembaga Petani" icon={ClipboardCheck} score={result.profileScore} anomalyCount={failed} defaultOpen={failed > 0}>
