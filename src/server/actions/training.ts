...
export async function createTrainingActivity(input: TrainingActivityInput) {
  ...
  // Pastikan lembaga petani target berada dalam scope data-access user.
  ...
  if (!targetGroup) {
    return { success: false, error: "Tidak memiliki izin untuk menambah pelatihan pada lembaga petani ini" };
  }
...
export async function updateTrainingActivity(input: UpdateTrainingActivityInput) {
  ...
  if (!targetGroup) {
    return { success: false, error: "Tidak memiliki izin untuk memindahkan pelatihan ke lembaga petani ini" };
  }
...
export async function getFarmersByGroup(farmerGroupId: string) {
  ...
  // Pastikan lembaga petani berada dalam scope data-access user sebelum
  ...
  if (!group) {
    throw new Error("Lembaga Petani tidak ditemukan atau tidak dalam akses Anda");
  }
...
export async function addParticipants(
  ...
  // milik lembaga petani activity tersebut (cegah injeksi farmerId sembarang).
  const access = await getAccessContext();
  const activity = await prisma.trainingActivity.findFirst({
  ...
  if (validFarmers.length !== farmerIds.length) {
    return { success: false, error: "Terdapat petani yang tidak valid untuk lembaga petani pelatihan ini" };
  }
