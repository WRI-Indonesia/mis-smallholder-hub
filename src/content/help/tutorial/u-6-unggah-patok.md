...
- **ZIP shapefile titik (Point)** — koordinat diambil dari geometrinya (WGS84); atribut DBF seperti `parcel_id`, `no`, `cond`, `type`, `installed` dikenali otomatis, sisanya bisa dipetakan manual.
...
+ **Patok baru** — nomor belum ada (atau kosong → nomor berikutnya). Titik yang berjarak ≤ 5 m dari patok lahan tetangga akan **ditautkan** ke patok itu, bukan dibuat baru.
