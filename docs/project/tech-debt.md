...
**Pembaruan per 2026-08-20** (review kode pasca-rilis v0.27.0, #278): **bug open 1 → 6** — **#237** (lama) · **#270** drift checksum migrasi lokal · **#273 `priority: high`** font Acumin berlisensi ter-commit & tersaji publik di repo PUBLIC (butuh keputusan owner; 4 berkas `.otf` bahkan tidak dipakai kode) · **#274** `countHotspotsByGroup` satu baris per baris boundary (laten — tidak ada unique constraint DB) · **#275** `geojson` boundary di-cast tanpa validasi · **#277** `deploy-staging.yml` tanpa `migrate deploy`. Debt aktif tetap **13** — tidak ada TD baru dibuka; kelima temuan di atas dilacak sebagai issue, bukan debt.
