...
### TD-045 · 🔲 Open — Migrasi prod bisa mendahului rilis kode yang memakainya; tak ada gate yang menangkap jendela itu (P2)

- **Masalah:** 2026-09-23 constraint `tbl_land_parcel_external_id` di **mis-prod** diubah (`20260923120000_external_id_shared_code`, #373) untuk mengimpor 142 kode sisa, sementara **aplikasi prod masih v1.0.0** yang menulis lewat unique lama `source_code`. Selama jendela itu jalur reaktivasi kode di aplikasi lama bisa salah sasaran, dan `mis-staging` (belum dimigrasi) akan galat bila dideploy kode baru. Pre-Commit Gate dan QA rilis sama-sama mengandaikan "migrasi ikut rilis"; migrasi yang diterapkan **di luar** rilis untuk kebutuhan data tidak punya jalur sendiri.
- **Validation:** aturan di `docs/standards/versioning.md` §Alur Rilis / `migrations.md`: migrasi prod di luar rilis hanya bila (a) **kompatibel mundur** dengan kode yang sedang jalan (dibuktikan: cari semua pemakai unique/kolom lama), atau (b) rilis kode menyusul **hari yang sama** — dicatat sebagai baris "jendela terbuka" di sprint sampai tertutup; plus cek otomatis `scripts/migrations/` yang membandingkan `applied-checksums.json` prod dengan tag rilis terakhir dan memperingatkan migrasi prod yang belum ada di tag. · **Evidence:** #373, changelog 2026-09-23. · **Issue:** #376. · **Owner:** Dev/Owner.
...
### TD-043 · 🔲 Open — Dua implementasi point-in-polygon yang wajib tetap setara, dijaga hanya oleh test (P3)

- **Masalah:** sejak #280 ada **dua** jalur uji titik-dalam-poligon di `src/lib/fire-alert.ts`: `pointInMultiPolygon` (dipakai `findContainingBoundaries` — klasifikasi lembaga) dan `pointInIndexedArea` + `indexRing` (dipakai klip provinsi & rekap per wilayah). Keduanya **disengaja** — indeks pita lintang menang telak pada satu area besar (309 → 18 ms) tetapi merugikan pada 31 boundary kecil (3,96 → 4,63 ms), keduanya terukur. Yang belum dijaga: **tak ada apa pun selain test yang memaksa keduanya memberi jawaban sama.** Risikonya sudah terbukti sekali — review pra-rilis v1.0.0 menemukan `indexRing` melewatkan sisi penutup pada ring yang datang tak tertutup, sementara `pointInRing` menutupnya secara implisit; selisihnya diam, dan itu persis kelas kehilangan titik yang #280 perbaiki. Regression test pertamanya pun **lolos walau bug-nya ada** (memakai persegi, yang sisi penutupnya jatuh di tepi bbox) dan harus ditulis ulang dengan belah ketupat.
- **Validation:** satu property-based test yang mengundi ratusan titik terhadap beberapa bentuk (berlubang, banyak pulau, ring tak tertutup, kolinear) dan menuntut `pointInMultiPolygon` ≡ `pointInIndexedArea` — menggantikan kumpulan kasus yang dipilih tangan, yang terbukti bisa memilih kasus yang tidak membedakan. Alternatif lebih murah: jadikan `pointInMultiPolygon` pembungkus tipis `indexArea` sekali-pakai, sehingga hanya ada satu ray casting; perlu diukur ulang karena itu memindahkan biaya indeks ke jalur boundary. · **Evidence:** #280, review pra-rilis v1.0.0 (`src/lib/fire-alert.ts`, `src/test/fire-alert.test.ts`). · **Owner:** Backend.

### TD-042 · 🔲 Open — Import form survei Monev BMP mem-parse Excel di browser: ~5 s per berkas, 192 berkas ≈ 5 menit (P2)

- **Masalah:** tab "Form survei per petani" (#346) membaca tiap `.xlsx` dengan `exceljs` **di klien** (`readXlsxWorkbookRaw`, 3 berkas paralel + progress bar) supaya pratinjau bisa dikoreksi sebelum dikirim. Satu form 3 sheet ± 5 s di laptop owner → batch satu Lembaga (± 25 form) ± 45 s, seluruh Rohul (192) ± 5 menit dengan tab browser sibuk; di perangkat lapangan lebih lambat lagi. Payload yang dikirim ke Server Action tetap kecil (skor saja), jadi bottleneck murni parsing.
- **Validation:** pindahkan parsing ke server (Server Action menerima berkas lewat S3 presigned seperti bulk upload lahan, atau `FormData` ≤ body limit) memakai `parseBmpSurveyForm` yang sama (lib murni — tidak perlu diubah), kembalikan pratinjau; atau Web Worker di klien bila ingin tetap offline-friendly. Ukur ulang dengan 192 form Rohul: target < 60 s total. · **Evidence:** #346 (`src/app/(admin)/admin/master-data/bmp-monev/bmp-survey-import-panel.tsx`). · **Owner:** Frontend/Backend.

### TD-041 · 🔲 Open — Helper gambar peta PDF tiga salinan: `farm-passport` / `layer-report-pdf` / `report-land-parcel` (P3)

- **Masalah:** `exteriorRings`, `strokeRing`, dan dekorasi peta (skala batang + panah utara) hidup di **tiga** berkas dengan semantik yang sudah menyimpang diam-diam: versi Profil Lahan (`src/lib/farm-passport.ts`) membuang titik penutup ring dan memakai daftar kandidat skala sampai 1 km, versi Laporan layer (`src/lib/layer-report-pdf.ts`) mempertahankan ring apa adanya, punya guard `< 2` titik, gaya `"F"`, dan label km; `report-land-parcel.ts` mengekspor `exteriorRings` ketiga. `farm-passport.ts` sudah mengimpor `drawGraticule` dari `layer-report-pdf.ts`, jadi arah ketergantungannya sudah ada — hanya helper lainnya yang belum ditarik. Ditemukan review #339 (2026-09-15); **tidak diubah saat review** karena helper `layer-report-pdf` bersifat privat dan menyatukannya mengubah hasil gambar PDF Profil Lahan (perlu verifikasi visual, bukan hanya `pdfText`).
- **Validation:** ekspor `exteriorRings`/`strokeRing`/`drawDecorations` dari `layer-report-pdf.ts` (atau modul `lib/pdf-map.ts` baru), parameterkan perbedaan yang disengaja (buang titik penutup, kandidat skala), ganti pemakaian di `farm-passport.ts` & `report-land-parcel.ts`, lalu bandingkan PDF Profil Lahan sebelum/sesudah pada 3 lahan uji (HJP kecil, tetangga besar, patok GPS di luar margin). Sekalian: `parcel-passport-query.ts` menyalin select `parcels` bertingkat yang sama dengan `MARKER_EXPORT_PARCEL_SELECT` (#339 temuan 14). · **Evidence:** review #339 temuan 12; `src/lib/farm-passport.ts` §`exteriorRings`/`strokeRing`/`drawMapDecorations`. · **Owner:** Frontend.
...
- **Tambahan 2026-09-23 (#373):** sejak satu kode boleh menempel di >1 lahan, klaim ganda membuat **kedua** lahan terhitung "sudah didata" (prod: 82 kode dipakai >1 lahan) — lahan yang kodenya keliru ikut masuk penyebut persentase legalitas sampai dicek silang. Makin menguatkan penanda pendataan eksplisit.
