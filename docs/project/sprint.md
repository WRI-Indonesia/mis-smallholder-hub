...
| #148 DASH: Card "Total Kelompok Tani" (distinct `subGroupLv2`) | DASH | 🟢 Code done | - | 2026-07-14 | **Kode ✅** — agregasi distinct `subGroupLv2` **per Lembaga** (ternormalisasi trim/case, null diabaikan, year-independent) di `dashboard-aggregation.ts` (`KTDetails.kelompokTaniCount` + `stats.totalKelompokTaniLahan`; `sumKelompokTaniStats`/`scopeSnapshotData`/`normalizeSnapshotData` ikut) + select `subGroupLv2` di `dashboard-query.ts` + kartu "Total Kelompok Tani" di `summary-cards.tsx` + 2 unit. Snapshot-backed: **kartu 0 sampai snapshot baru di-generate** (belum regen; DB). **Depends:** data `subGroupLv2` terisi (#150); pindah ke `count` tabel KT saat TD-014. Gate lint 0 / build / test 398 ✅ |
