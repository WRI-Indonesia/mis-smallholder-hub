...
- **Skema:** `BmpAssessment` → `tbl_bmp_assessment` (migrasi `20260918120000`, manual dari `migrate diff`, additive). Kategori **tidak disimpan** — `BMP_ASSESSMENT_CATEGORIES` di `lib/bmp-assessment.ts`. Satu baris aktif per petani-tahun dijaga di action.
