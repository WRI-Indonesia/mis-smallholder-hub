...
| Tests          | `npm test` lulus **47 test files / 722 tests** ✅ (verifikasi 2026-07-31); domain: auth & middleware, RBAC (rbac, rbac-permission, rbac-server-guards, access-context), menu (action/filter/tree), user (action/data-access/menu-access), region, farmer (+detail, +sub-groups), farmer-group (+detail), land-parcel, training (activity/participant), production, bulk-upload (+parcel-mapping), report (petani/pelatihan/produksi/kelompok-tani ×2/lahan ×3), dashboard (main/bmp/training/asymmetry/data-availability), data-analyst, data-completeness, map (map, map-geo, firms), help (content/media), pdf-exporters, perf | Testing solid untuk semua core features; gap tersisa: integration test route hotspot — rincian per-issue di OPS-01 |
