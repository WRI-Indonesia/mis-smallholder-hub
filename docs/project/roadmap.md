...
- **Next step:** Maintain; **5 migrasi `20260914*` applied `mis-dev` + `mis-staging-local` (09-14) + `mis-staging` (09-15, #333), belum prod** (satu siklus deploy + seed menu `report-marker` + refresh `applied-checksums.json` #303); expand to Production dependency. Fase 2 pohon: layer titik di Peta Lahan. #317 Fase 2 (deteksi tumpang tindih) kini bisa langsung memakai `geom`.
