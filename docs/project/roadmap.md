...
- **#365 ✅ (2026-09-22): laporan bulanan titik api dari arsip FIRMS** — mode **Bulan tertentu** di toggle rentang (Select Bulan + Tahun, Jan 2020 … bulan berjalan; masuk = bulan lalu; bulan berjalan = parsial s.d. hari ini): peta, kartu, keyakinan, tabel lembaga & PDF mengikuti periode. Proxy `?month=YYYY-MM` (eksklusif dengan `dayRange`): `monthWindows` memetakan tiap hari ke sumber — arsip **`VIIRS_SNPP_SP`** diutamakan, **NRT** untuk yang belum terarsip — lalu jendela 5 hari dari tanggal 1 (±7 request/bulan); batas SP/NRT dibaca dari `data_availability` (cache 6 jam), hari tanpa sumber → `coverage.missingDates` (FIRMS memangkas jendela di luar ketersediaan **tanpa galat**, diverifikasi live); bulan lampau `revalidate` 30 hari. PDF **"Laporan Bulanan Titik Api"**: meta Periode, **Tren Harian** (UTC, bar porsi, puncak merah, tanggal kosong "tidak tersedia"), **Rekap per Kabupaten** (+Total), **Rekap per Lembaga**, metodologi ber-sumber (`describeHotspotSources`) + tanggal kosong; detail tetap lengkap (#287). Keputusan owner 2026-09-22: arsip on-demand (bukan snapshot DB #271), bulan kalender saja, halaman yang ada, sejak 2020. Test +30 (`monthWindows`/`parseDataAvailability`/route mode bulan/`countHotspotsByDay`/`summarizeByNamedArea`/PDF bulanan); Bantuan p-11 langkah 6 + troubleshooting.
...
- **#360 ✅ (2026-09-22):** definisi **Menerapkan BMP** diperluas ke Perintis (skor ≥ 1,00; hanya Belum Implementasi yang belum) di kartu KPI, label batang, dan KPI Master Data › Monev BMP; `description` arti tiap kategori di `BMP_ASSESSMENT_CATEGORIES` (tooltip ubin dashboard, badge Profil Petani #343); nama kegiatan 1.1 "Training" → "Knowledge" di CSV seed indikator (seed parsial mis-dev ✓; staging/prod ikut deploy berikutnya).
