...
**Last updated:** 2026-07-08 (#99 DASH-01 Dashboard Snapshot selesai: Main Dashboard `/admin/dashboard/main` + peta MapLibre + snapshot module `/admin/tools/snapshot` + `MainDashboardSnapshot` model/migration; test lokal 19 files / 216 tests ✅. Sebelumnya: #107 RPT-01 & #108 RPT-02 ✅; #109 RPT-03 siap dikerjakan. Baru: stream `MAP` + #113 [MAP-01] Map/Peta Lahan dibuat — scaffolding, Not Started)
...
| 🔲 Not Started | 4 fase         | CMS-01, COMM-01, RPT-03 (issue #109 sudah dibuat), MAP-01 (issue #113 sudah dibuat) |
...
| Admin routes   | ✅ Dashboard, Settings (Users/Roles/Menu/Regions), Master Data (Farmers list/detail/form + Groups list/detail/form + Parcels list/detail/form + Training list/detail/participants + Production list/detail/form), Bulk Upload (Farmers + Parcels Shapefile + Production), Profile, Data Analyst (Farmer Summary) | ✅ Admin foundation complete + Farmer CRUD + Land Parcel CRUD + Training CRUD + Production CRUD + Data Analyst (Farmer Summary) complete; Dashboard placeholder only |
| Server actions | `user` (259 LOC), `user-data-access` (193 LOC), `user-menu-access` (195 LOC), `menu` (110 LOC), `region` (104 LOC), `farmer-group` (145 LOC), `farmer` (188 LOC), `land-parcel` (165 LOC), `bulk-upload-parcel` (222 LOC), `profile` (47 LOC), `role-permission` (74 LOC), `bulk-upload` (177 LOC), `training` (355 LOC), `production` (180 LOC), `bulk-upload-production` (95 LOC), `upload` (77 LOC), `data-analyst` (140 LOC) — **Total 2697 LOC** | Training, farmer, land parcel, production, bulk upload, data analyst tersedia ✅; dashboard belum ada |
...
| Tests          | `npm test` lulus **17 test files / 203 tests** ✅; test files: auth, bulk-upload, bulk-upload-parcel, data-analyst, farmer, land-parcel, menu-action, middleware, perf, production, rbac, rbac-permission, region, training-activity, training-participant, user-action, user-data-access, user-menu-access | Testing solid untuk core features (auth/RBAC/menu/user/region/farmer/land-parcel/training/production/bulk-upload/data-analyst); need dashboard/report |
...
| DA       | Data Analyst           | Farmer Summary, analytics dashboards                                                       |
...
| DASH-03     | Interactive Map              | ✅ Done        | Done    | `src/app/(admin)/admin/dashboard/dashboard-map.tsx` — MapLibre (react-map-gl) clustered KT markers, auto-fit bounds, click-to-select info panel, NULL-coordinate empty state ✅ | Implement #99 completed |
...
| MAP-01      | Map: Peta Lahan              | 🔲 Not Started | Next    | **#113 dibuat 2026-07-08** (scaffolding): menu `map` + `map-parcel` (`/admin/map/parcel`), peta full-bleed MapLibre (`react-map-gl`) + panel filter floating (Province/District/KT) + legend layer toggle (point KT / centroid lahan / polygon lahan), read-only dari `FarmerGroup`+`LandParcel` (tanpa tabel baru); centroid via `@turf`. Belum ada route/action/UI. | Implement #113: `src/server/actions/map.ts` + `/admin/map/parcel` UI + `src/test/map.test.ts`; dependency MD-02, MD-04 |
