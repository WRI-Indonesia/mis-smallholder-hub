...
- **`BmpIndicator` (`ref_bmp_indicator`)** — master 32 indikator dari form survei tim lapangan: 5 kegiatan berbobot (1.1 Knowledge 0,10 — semula "Training", diganti owner 2026-09-21 lewat seed · 1.2 Pemupukan 0,35 · 1.3 Pengendalian Gulma 0,10 · 1.4 PHPT 0,10 · 1.5 Panen 0,35), kode `1.2.3.2` = kriteria + no urut, level **LEMBAGA** (14) / **INDIVIDU** (18), `weight` (21 berbobot; null = informatif, `inFinalScore=false`), rubrik `scoreLabel0..3`. Σ bobot indikator per kegiatan = 1,0 kecuali Gulma yang mentah 1,2 karena kriteria **1.3.2 Identifikasi Gulma bersifat alternatif** — petani ATAU pekerja (0,2 masing-masing), hanya salah satu yang ditanya (owner 2026-09-20, "maksimal harus 3"); aturan ini di kode (`BMP_EXCLUSIVE_CRITERIA` di `lib/bmp-survey-form.ts`, bukan kolom): hitung ulang memakai skor tertinggi yang terisi dan bobotnya sekali → Σ efektif 1,0, maks 3,00. Rumus form menjumlahkan keduanya (13/192 form Rohul terisi dua-duanya → total form bisa > hitung ulang). Di-seed dari `prisma/seeds/data/bmp-indicators.csv` (dibangkitkan dari sheet Panduan; tanpa PII) lewat `seed-bmp-indicators.ts` (upsert by `(code, level)`; skrip parsial `scripts/seed/seed-bmp-indicators.ts`).
- **`BmpAssessmentDetail`** — skor indikator INDIVIDU per penilaian petani (`score` Int? null = tidak dinilai; `weightUsed` salinan bobot saat disimpan supaya revisi rubrik tak mengubah riwayat; `notes`). Unik per `(assessmentId, indicatorId)`, di-upsert utuh.
- **`BmpGroupAssessment` + `BmpGroupAssessmentDetail`** — penilaian LEMBAGA per tahun (14 indikator). Satu aktif per Lembaga-tahun (partial unique tulis tangan). Enam indikatornya berbobot dan **masuk skor akhir setiap petani** Lembaga itu — karena itu `recomputeBmpScore` membaca rincian individu + penilaian Lembaga tahun yang sama.
- **Rumus form:** skor akhir = Σ_kegiatan bobot × Σ_indikator (bobot × skor), sel kosong = 0. **`BmpAssessment.score` = angka resmi.** Import form survei menyimpan **hitung ulang sistem** (bukan total raport form — rumus form menjumlahkan kriteria alternatif, dan 13 dari 192 form Rohul totalnya basi terhadap sheet indikator); total form hanya dipakai bila berkas tanpa rincian. Import rekap Excel menyimpan angka rekap apa adanya (tanpa rincian, tak bisa dihitung ulang). Di halaman detail, hitung ulang untuk verifikasi & raport per kegiatan — selisih > 0,01 ditampilkan sebagai peringatan; menimpa `score` hanya lewat centang eksplisit di grid skor indikator.
- **Import form survei** (`importBmpSurveyForms`): identitas = **nama berkas** (header form tak dipercaya — 8 berkas Rohul header ≠ nama file), fuzzy ke petani Lembaga (`matchFarmerName`: EXACT/FUZZY/AMBIGUOUS/NONE) + dropdown di pratinjau; skor indikator dibaca dari sheet Survey Lembaga/Individu (bukan formula Gabungan yang cache-nya bisa kosong), dipetakan ke master lewat kriteria + awalan teks dengan fallback urutan dalam kriteria; skor di luar 0–3 diterima (Zod import 0–9) dan ditandai; penilaian Lembaga per tahun dari berkas pertama (berkas lain berbeda → peringatan).
