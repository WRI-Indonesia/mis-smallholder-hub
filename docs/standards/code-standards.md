...
**Tabel satelit** (atribut yang punya siklus hidup sendiri, bisa >1 per induk, atau keikutsertaan program — bukan sifat intrinsik induk) dinamai **`tbl_<induk>_<aspek>`** dengan nama induk **lengkap** persis seperti tabel intinya, model `<Induk><Aspek>`. Preseden: `tbl_farmer_group_boundary`, `tbl_training_participant`. Contoh rencana untuk lahan (Decision Log 2026-08-27): `tbl_land_parcel_document` (SHM/SKT/SKGR dkk., 1:N), `tbl_land_parcel_external_id` (kode pemetaan vendor + poligon mentah opsional, 1:N), `tbl_land_parcel_program` (demplot/PBU).
