...
- **Perbedaan isi vs prod sejak 2026-09-01:** `crop_type` seluruh lahan di `mis-dev` & `mis-staging-local` sudah diseragamkan **"Kelapa Sawit"** (13.668 baris, 9.505 di antaranya semula NULL); **`mis-staging` & `mis-prod` belum** — tunnel tertutup saat pengerjaan. Sampai keduanya menyusul, skor Ketersediaan Data domain Lahan di lokal akan lebih tinggi daripada prod. Backup kolom: `tmp-backup/croptype-<db>-before-<ts>.csv`.
