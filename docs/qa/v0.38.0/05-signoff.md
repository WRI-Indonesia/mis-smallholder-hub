...
| QA | — | 2026-09-22 | **Terbatas** | Peran OPERATOR/DONOR (TC-343-04, SM-28/29) belum dijalankan — butuh owner login akun QA lokal; unduhan Excel/PDF laporan & 6 regresi berkas uji tak diulang (tak berubah sejak run prod v0.37.0). Staging: deploy `5ca0d2c` **hijau** (run 35698042656, 3m56s) setelah 2× OOM type-check (`80a0fe6`) → type-check dipisah dari build; smoke UI staging butuh URL/akun; **seed #361 belum applied** (tunnel DB belum aktif) |
| Owner | Sofyan | 2026-09-22 | **Go** | "lanjut" (2×): seed #361 ke prod + merge PR #362 `staging → main`; known issues #363 (memori server), #364 (akun demo ubah menu prod), #311 (perf flaky) diterima |
