...
| DASH-06 | Dashboard Pelatihan | **Live query (bukan snapshot)** `/admin/dashboard/training` (dirombak #198): 4 KPI card satu-angka-besar (Petani Terlatih, Total Sesi, Partisipasi Perempuan, Kenaikan Skor; pola StatEmph) + card collapsible **Capaian Paket per Distrik** (paket × distrik, kolom Total (Riau), stacked bar sudah/belum) + matriks **Capaian Paket per Lembaga** (heatmap sortable, collapsible, lebar kolom seragam) + tren stacked-bar per paket + panel efektivitas pre/post + panel kualitas data ("Sesi tanpa …") ber-deep-link; **drill-down sel → daftar petani belum dilatih (salin/Excel)**; target 100% per paket; terminologi **Sesi**; filter Kategori/Distrik/Lembaga/Tahun client-side |
