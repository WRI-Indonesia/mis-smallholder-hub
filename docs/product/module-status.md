...
| MD-03 | Farmers | Full CRUD, RBAC, joinedYear; **detail = profil 360° ber-Tabs** (5 cards incl. Kelengkapan Profil & Produktivitas, lahan + peta shared + PDF Profil Lahan #134, checklist paket, produksi per tahun/bulanan) (#172); **PDF Profil Petani** (#343): ringkasan petani + lampiran Profil Lahan tiap lahan (+ Monev BMP bila VIEW), tombol header detail & aksi baris daftar (izin PRINT), dialog Lengkap/Ringkasan bila lahan > 10 |
...
| DASH-08 | Monev BMP (#344) | **Live query** `/admin/dashboard/bmp-monev` (pola Pelatihan; terpisah dari BMP Dashboard (Produksi)): 4 KPI (Petani Dinilai / Rerata Skor / Menerapkan BMP = Perintis+Praktisi+Teladan (#360) / Lembaga Tercakup) + komposisi kategori per Lembaga (stacked 100%) + ranking rerata Top-10 (skala tetap 0–3) + tren per tahun survei + tabel rekap per Lembaga ber-Excel; filter URL Distrik/Lembaga/Tahun (default tahun terbaru ber-data). Data: Master Data › Monev BMP (`/admin/master-data/bmp-monev`: DataTable, form, **import Excel format rekap** ber-pratinjau, upsert per petani-tahun) + tab Monev BMP di Detail Petani. Kategori Teladan > 2,50 · Praktisi ≥ 1,50 · Perintis ≥ 1,00 · Belum < 1,00 dihitung dari skor |
| DASH-07 | Fire Alert (Risk Management) | Dashboard `/admin/dashboard/risk/fire` (#266): titik api VIIRS/FIRMS se-Riau diklasifikasi **point-in-polygon** terhadap boundary lembaga (`FarmerGroupBoundary`, seed dari shapefile ICS); ¾ peta (boundary ungu #660099, titik dalam = ikon api / luar = lingkaran, klik baris tabel → zoom + highlight, garis batas kabupaten BIG dari `AdministrativeBoundary`) + ¼ panel (kartu ringkasan, confidence, tabel Lembaga × titik — hanya lembaga ber-titik); toggle rentang 24 jam/5/10/30 hari (#284 — >5 hari = gabungan jendela 5 hari ber-`DATE`, tanpa riwayat DB); **Print Map PDF** Full Riau/per distrik (auto-zoom + tabel rekap); live FIRMS tanpa riwayat DB; scope mengikuti access context |
| MAP-01 | Map: Peta Lahan | Peta full-bleed + overlay referensi pemerintah (Kawasan Hutan & Gambut, #215) + custom GIS + hotspot FIRMS (rentang 24 jam/5/10/30 hari, #284) + ruler + label (#113); produksi popup real + PDF "Profil Lahan" matriks (#134); panel daftar lahan search+zoom (#135); legenda collapsible; aksi popup **Lihat Detail**/**Edit Lahan** ber-gate permission via primitif bersama `map-popup.tsx` (#188); revamp Titik Api (#240): 24 jam bergulir, breakdown keyakinan, klik label = zoom, ekspor SHP/PDF < 15 km + modal ringkasan |
...
| Bantuan: registrasi berkas ↔ `CHAPTER_SOURCES`, frontmatter ↔ `menu.csv`, cakupan tutorial **37/37** tanpa pengecualian tersisa (#257 tuntas 2026-09-23; `TANPA_TUTORIAL` kosong — baris baru di sana = menu dirilis tanpa materi pengguna) | help-registry.test.ts | 10 | ✅ |
...
| Hotspot FIRMS (+ pemecah jendela DATE #284) | firms.test.ts | 24 | ✅ |
| Hotspot route `/api/map-hotspot` (#231, #284) | map-hotspot-route.test.ts | 14 | ✅ |
| Overlay tile route `/api/map-overlay/[key]` (#241) | map-overlay-route.test.ts | 8 | ✅ |
| Hotspot klien (24 jam bergulir + keyakinan + label/usia/nama berkas) | map-hotspot.test.ts | 11 | ✅ |
