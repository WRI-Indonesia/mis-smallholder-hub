...
| Tab Lahan — `Daftar Lahan (n)` | Tabel | `Kode Lahan` (link detail lahan **tab baru** — pola #224, gate `canViewParcel`), `Kelompok Tani`, `Blok`, `Surat` (ringkasan `JENIS nomor` dokumen aktif, #296), `STDB` (nomor, #296), `NKT` (badge merah/amber bila termasuk/terdampak, "Tidak" bila dinilai bersih, "—" belum dinilai, #330), `Luas (Ha)`, `Tahun Tanam`, `Jumlah Pohon` (dihitung di page dari `getFarmerTreePoints`, "—" bila belum ada, #238/#241), `Revisi`, `Profil Lahan`; empty state `Petani ini belum memiliki lahan.` |
