...
| Profil Lahan | Tombol popup | Generate PDF Farm Passport (`src/lib/farm-passport.ts`) via `getParcelPassport` — komposisi (#298, rombak total): **hal. 1** header ber-ID lahan besar + badge PSR/komoditas + tanggal cetak → 4 kartu ringkasan (Luas · Legalitas · Tanaman · Produksi, cermin halaman web) → Layout Lahan (peta 56% kiri, label huruf ID; sejak **#327** lahan tetangga ≤ 25 m putus-putus bernomor, lahan utama tetap dominan — tetangga besar dipotong di bingkai, skala batang + panah utara, lalu **legenda tetangga** No · Pemilik · ID Lahan · Lembaga · Jarak di bawah peta, cap 12 + "+N lahan lain"; nama petani tetangga di luar scope pencetak diganti "—", Lembaga tetap — pengecualian scope tercatat di `access-context.md`) + Informasi Lahan & Pemilik & **Sepadan** (#326, grid 2×2 U/T/S/B, "—" bila kosong) (kanan) → **Legalitas & Dokumen** (tabel surat: jenis/nomor/nama/luas tertera/selisih vs poligon/terbit/keterangan; tabel STDB + "Juga mencakup"; baris UL Parcel Code & program), **hal. 2** Pelatihan, Produksi (pivot bulanan + Total + **Ton/Ha**); footer + nomor halaman di tiap halaman; label proses "Menyiapkan..."; gagal → toast "Gagal membuat PDF profil lahan" — digate izin `PRINT` menu Peta Lahan `map-parcel` (#245) — sama dengan guard `getParcelPassport`; varian dari halaman Lahan/Petani tetap digate menu masing-masing |
