...
| Profil Lahan | Tombol popup | Generate PDF Farm Passport (`src/lib/farm-passport.ts`) via `getParcelPassport` — isi (#298): header + identitas petani, Layout Lahan (poligon, luas, titik tengah, tahun tanam, jumlah pohon), Data Petani + Informasi Lahan (blok/KT, status, komoditas+species, tahun tanam+PSR), **Legalitas & Dokumen** (tabel surat: jenis/nomor/nama/luas tertera/selisih vs poligon/terbit/keterangan; tabel STDB + "Juga mencakup"; baris kode vendor & program), Pelatihan, Produksi; boleh >1 halaman, footer + nomor halaman di tiap halaman; label proses "Menyiapkan..."; gagal → toast "Gagal membuat PDF profil lahan" — digate izin `PRINT` menu Peta Lahan `map-parcel` (#245) — sama dengan guard `getParcelPassport`; varian dari halaman Lahan/Petani tetap digate menu masing-masing |
