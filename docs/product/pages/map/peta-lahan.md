...
| Legenda | Section collapsible + Legend | Muncul hanya setelah data dimuat; tiap baris = checkbox toggle layer + swatch warna + jumlah fitur + **ikon zoom ke layer** (crosshair, permintaan owner 2026-09-14 — klik teks label juga zoom, `LayerZoomTarget`) + (izin EXPORT) **ikon unduh per baris** (#331) |
| Unduh per baris legenda (#331) | Dropdown per baris (`LegendRow.onExport`) | Item **Excel** · **Shapefile (ZIP)** · **GeoJSON** · **KML** · **PDF** (izin PRINT; `buildLayerReportDoc` — **A4 landscape**: peta ikhtisar + tabel atribut multi-halaman; lahan lain sebagai konteks **ungu 12 %**, lahan NKT merah/amber; nomor fitur berlatar putih dengan **anti-tumpuk + garis penunjuk**; bila fitur < 6 mm di ikhtisar → **halaman peta rinci per klaster** (single-linkage 8 % bentang) berlabel A/B…; > 200 fitur → tanpa nomor); format spasial mengikuti **tipe fitur baris** (label kecil "Point"/"Polygon" di item): Point Lembaga → Point (kode/nama/distrik); Point Lahan → Point centroid + atribut lengkap lahan; Area Lahan → Polygon (= "Unduh Lahan" lama); Lahan NKT → Polygon hanya lahan INCLUDED/AFFECTED (atribut ekspor kini memuat kolom `nkt`); Patok / Patok NKT → **satu baris/fitur per patok fisik** (`uniqueMarkerRows`, keputusan owner 2026-09-14): kolom Kode Patok + "Lahan (Nama Petani · ID Petani · ID Lahan #no)" satu lahan per baris (wrapText), urut Kelompok Tani → Blok; **PDF patok** memakai tabel **per lahan** (Nama Petani · ID Petani · ID Lahan dipecah kolom, kolom No peta · Kode Patok · No di lahan · Kondisi masing-masing satu patok per baris sejajar — permintaan owner: baris per patok mengulang nama petani); pada **Patok NKT** kolom itu hanya memuat lahan yang kena NKT (`parcelNkt`), pemakai bersih tetap dihitung; `getMapMarkerExportRows`, gate `map-parcel:EXPORT`. Helper klien `map-legend-export.ts`, unduhan generik `downloadFeatureExport` (SHP Point/Polygon, DBF-safe per dataset); nama file `<baris>-<label>-<stempel>` |
...
| Ekspor titik api | Tombol | "Unduh SHP" (ZIP Shapefile point WGS84 via `@mapbox/shp-write`, kolom DBF-safe ≤10 char) & "Cetak PDF" (jsPDF landscape: ringkasan total + per keyakinan + jumlah < 15 km dari Lembaga Petani; tabel hanya titik < 15 km, kolom Lembaga Terdekat & "Jarak (km) & Arah dari Kantor Lembaga" (mis. `2,49 · B 287°`, #293), urut jarak terdekat; header "Data lahan: <wilayah>" (field kosong dihilangkan, bukan "—") dan ringkasan total ber-keterangan "se-Riau" (#294); tanpa data peta dimuat → semua titik, jarak "—"); disabled saat layer mati/loading/0 titik. Kalkulasi jarak lazy di background (`calcHotspotNearest`, chunked 250 titik/tick agar peta responsif) — tombol PDF spinner+disabled selama menghitung, toast "Kalkulasi jarak titik api selesai — Cetak PDF siap" saat siap. Helper `map-hotspot-export.ts`. "Unduh SHP" digate izin `EXPORT`, "Cetak PDF" digate izin `PRINT` (#245) |
| Ringkasan titik api | Modal (`map-hotspot-summary.tsx`) | Auto-terbuka saat kalkulasi jarak selesai; buka ulang via tombol "Lihat Ringkasan" (disabled selama menghitung). Subtitle: rentang + "Data lahan: <wilayah>" + sumber, dengan nilai ditebalkan (#294) — nama wilayah TIDAK ditulis berjajar polos karena modal mencampur dua cakupan: hitungan titik api selalu se-Provinsi Riau, sedangkan "< 15 km" relatif terhadap Lembaga Petani pada data lahan yang dimuat. Isi: dua kartu angka ber-ikon ("Total titik se-Riau", "< 15 km dari Lembaga") + bar bertumpuk sebaran keyakinan dengan legenda berangka (opsi C, keputusan owner 2026-08-24; segmen bukan-nol diberi lantai 3px agar nilai kecil tidak lenyap); tabel hanya titik < 15 km urut jarak terdekat (No, Waktu WIB, Satelit, badge Keyakinan berwarna legenda, FRP, Lembaga Terdekat, "Jarak (km) & Arah dari Kantor Lembaga"); klik baris = tutup modal + zoom peta ke titik (`pointZoomRequest`, zoom 14); footer tombol Unduh SHP & Cetak PDF (gating sama: izin `EXPORT`/`PRINT`, #245) |
...
| Profil Lahan | Tombol popup | Generate PDF Farm Passport (`src/lib/farm-passport.ts`) via `getParcelPassport` — komposisi (#298, rombak total): **hal. 1** header ber-ID lahan besar + badge PSR/komoditas + tanggal cetak → 4 kartu ringkasan (Luas · Legalitas · Tanaman · Produksi, cermin halaman web) → Layout Lahan (peta 56% kiri, label huruf ID; sejak **#327** lahan tetangga ≤ 25 m putus-putus bernomor, lahan utama tetap dominan — tetangga besar dipotong di bingkai, skala batang + panah utara, lalu **legenda tetangga** No · Pemilik · ID Lahan · Lembaga · Jarak di bawah peta, cap 12 + "+N lahan lain"; identitas tetangga selalu lengkap apa pun scope pencetak — pengecualian scope tercatat di `access-context.md`) + Informasi Lahan & Pemilik & **Sepadan** (#326, empat baris label+nilai U/T/S/B, kosong → satu baris "Belum diisi") (kanan) → **Legalitas & Dokumen** (tabel surat: jenis/nomor/nama/luas tertera/selisih vs poligon/terbit/keterangan; tabel STDB + "Juga mencakup"; baris UL Parcel Code & program), lalu Pelatihan, Produksi (pivot bulanan + Total + **Ton/Ha**) — **section mengalir sejak #327** (keputusan owner 2026-09-14): Pelatihan tidak lagi dipaksa mulai halaman 2, pindah halaman hanya bila sisa < 40 mm, biasanya total tetap 2 halaman; footer + nomor halaman di tiap halaman; label proses "Menyiapkan..."; gagal → toast "Gagal membuat PDF profil lahan" — digate izin `PRINT` menu Peta Lahan `map-parcel` (#245) — sama dengan guard `getParcelPassport`; varian dari halaman Lahan/Petani tetap digate menu masing-masing |
