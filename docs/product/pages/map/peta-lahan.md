...
| Profil Lahan | Tombol popup | Generate PDF Farm Passport (`src/lib/farm-passport.ts`) via `getParcelPassport` — komposisi (#298, rombak total): **hal. 1** header ber-ID lahan besar + badge PSR/komoditas + tanggal cetak → 4 kartu ringkasan (Luas · Legalitas · Tanaman · Produksi, cermin halaman web) → Layout Lahan (peta 56% kiri, label huruf ID) + Informasi Lahan & Pemilik (kanan) → **Legalitas & Dokumen** (tabel surat: jenis/nomor/nama/luas tertera/selisih vs poligon/terbit/keterangan; tabel STDB + "Juga mencakup"; baris UL Parcel Code & program), **hal. 2** Pelatihan, Produksi (pivot bulanan + Total + **Ton/Ha**); footer + nomor halaman di tiap halaman; label proses "Menyiapkan..."; gagal → toast "Gagal membuat PDF profil lahan" — digate izin `PRINT` menu Peta Lahan `map-parcel` (#245) — sama dengan guard `getParcelPassport`; varian dari halaman Lahan/Petani tetap digate menu masing-masing |
