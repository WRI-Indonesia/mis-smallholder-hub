...
| Lapis | Isi | Status |
|---|---|---|
| `tutorial` | Panduan **per tugas**, dua tingkat kedalaman (Ringkas/Detail) | 31 topik / 5 bab |
...
|---|---|---|---|---|
| 1 | tutorial | Mengelola Data Harian (`tutorial-data-harian`) | Langkah demi langkah pekerjaan yang paling sering dilakukan: mendaftarkan petani, lahan, pelatihan, dan produksi. | 1.1 Mendaftarkan Lembaga Petani (`menambah-lembaga`) · 1.2 Mendaftarkan petani baru (`menambah-petani`) · 1.3 Mendaftarkan lahan petani (`menambah-lahan`) · 1.4 Mencatat pelatihan & pesertanya (`mencatat-pelatihan`) · 1.5 Mencatat hasil panen (`mencatat-produksi`) |
| 2 | tutorial | Unggah Massal (`tutorial-unggah-massal`) | Memasukkan data dalam jumlah besar sekaligus — Excel untuk petani & produksi, shapefile untuk lahan. | 2.1 Mengunggah data petani dari Excel (`unggah-petani`) · 2.2 Mengunggah data produksi dari Excel (`unggah-produksi`) · 2.3 Mengunggah lahan dari shapefile (`unggah-lahan`) · 2.4 Mengunggah titik pohon sawit (`unggah-pohon`, #238) |
| 3 | tutorial | Memantau & Menindaklanjuti (`tutorial-memantau`) | Membaca dashboard dan peta, lalu mengubah temuannya jadi daftar kerja yang bisa ditindaklanjuti. | 3.1 Membaca Main Dashboard (`membaca-dashboard`) · 3.2 Menindaklanjuti petani yang belum dilatih (`cakupan-pelatihan`) · 3.3 Membaca peta lahan & peta BMP (`membaca-peta`) · 3.4 Membaca BMP Dashboard (produksi) (`dashboard-bmp`) · 3.5 Menjelajah Peta Lahan (`peta-lahan`) · 3.6 Memantau titik api (Fire Alert) (`fire-alert`, #266) · 3.7 Memeriksa ketersediaan data (`ketersediaan-data`) · 3.8 Membaca Ringkasan Petani (`ringkasan-petani`) · 3.9 Membandingkan data acuan (`komparasi-data-acuan`, #243) · 3.10 Membaca Metrik Rilis (`metrik-rilis`) · 3.11 Membaca Peta Data & Skema (`peta-data-skema`, #256) |
| 4 | tutorial | Laporan & Perawatan (`tutorial-laporan`) | Menyiapkan berkas untuk donor atau audit, dan menjaga angka dashboard tetap mutakhir. | 4.1 Menyiapkan laporan untuk dicetak (`membuat-laporan`) · 4.2 Mencetak Laporan Lahan ber-peta (`laporan-lahan`) · 4.3 Mencetak Laporan Pelatihan (`laporan-pelatihan`) · 4.4 Mencetak Laporan Produksi (`laporan-produksi`) · 4.5 Mencetak Laporan Kelompok Tani (`laporan-kelompok-tani`) · 4.6 Memperbarui angka dashboard (`memperbarui-dashboard`) |
