...
| Lapis | Isi | Status |
|---|---|---|
| `tutorial` | Panduan **per tugas**, dua tingkat kedalaman (Ringkas/Detail) | 42 topik / 5 bab |
...
| Frontmatter topik | `title` (wajib), `icon` (nama ikon lucide yang terdaftar di map `ICONS`; tak dikenal → `HelpCircle`), `intro` (opsional). `menuKey` + `permission` boleh juga dipakai topik **non-tutorial** — topik itu lalu ikut dihitung sebagai cakupan menu tersebut, tetapi **tidak** menjadi target tombol Panduan (`findTutorialForMenu` hanya memindai bab ber-`section: "tutorial"`) |
...
|---|---|---|---|---|
| 1 | tutorial | Mengelola Data Harian (`tutorial-data-harian`) | Langkah demi langkah pekerjaan yang paling sering dilakukan: mendaftarkan petani, lahan, pelatihan, dan produksi. | 1.1 Mendaftarkan Lembaga Petani (`menambah-lembaga`) · 1.2 Mendaftarkan petani baru (`menambah-petani`) · 1.3 Mendaftarkan lahan petani (`menambah-lahan`) · 1.4 Mencatat pelatihan & pesertanya (`mencatat-pelatihan`) · 1.5 Mencatat hasil panen (`mencatat-produksi`) · 1.6 Mencatat surat, STDB, dan program pada lahan (`mencatat-legalitas-lahan`, #296) · 1.7 Mengelola patok batas lahan (`mengelola-patok-lahan`, #329) · 1.8 Mencatat hasil Monev BMP (`mencatat-monev-bmp`, #344) · 1.9 Mengimpor form survei & membaca rincian indikator Monev BMP (`rincian-indikator-monev-bmp`, #346) |
| 2 | tutorial | Unggah Massal (`tutorial-unggah-massal`) | Memasukkan data dalam jumlah besar sekaligus — Excel untuk petani, produksi & detail lahan, shapefile untuk poligon lahan. | 2.1 Mengunggah data petani dari Excel (`unggah-petani`) · 2.2 Mengunggah data produksi dari Excel (`unggah-produksi`) · 2.3 Mengunggah lahan dari shapefile (`unggah-lahan`) · 2.4 Mengunggah detail lahan — surat, STDB, UL Parcel Code (`unggah-detail-lahan`, #296) · 2.5 Mengunggah titik patok dari GPS (Excel/CSV atau shapefile titik) (`unggah-patok`, #329) · 2.6 Mengunggah titik pohon sawit (`unggah-pohon`, #238) |
| 3 | tutorial | Memantau & Menindaklanjuti (`tutorial-memantau`) | Membaca dashboard dan peta, lalu mengubah temuannya jadi daftar kerja yang bisa ditindaklanjuti. | 3.1 Membaca Main Dashboard (`membaca-dashboard`) · 3.2 Menindaklanjuti petani yang belum dilatih (`cakupan-pelatihan`) · 3.3 Membaca peta lahan & peta BMP (`membaca-peta`) · 3.4 Membaca BMP Dashboard (produksi) (`dashboard-bmp`) · 3.5 Membaca Dashboard Monev BMP (`dashboard-monev-bmp`, #344) · 3.6 Menjelajah Peta Lahan (`peta-lahan`) · 3.7 Mengunduh data spasial lahan (`unduh-spasial-lahan`, #313) · 3.8 Memantau titik api (Fire Alert) (`fire-alert`, #266) · 3.9 Memeriksa ketersediaan data (`ketersediaan-data`) · 3.10 Membaca Ringkasan Petani (`ringkasan-petani`) · 3.11 Membandingkan data acuan (`komparasi-data-acuan`, #243) · 3.12 Membaca Metrik Rilis (`metrik-rilis`) · 3.13 Membaca Peta Data & Skema (`peta-data-skema`, #256) |
| 4 | tutorial | Laporan & Perawatan (`tutorial-laporan`) | Menyiapkan berkas untuk donor atau audit, dan menjaga angka dashboard tetap mutakhir. | 4.1 Menyiapkan laporan untuk dicetak (`membuat-laporan`) · 4.2 Mencetak Laporan Lahan ber-peta (`laporan-lahan`) · 4.3 Menyaring lahan yang belum punya surat atau STDB (`menyaring-legalitas-lahan`, #305) · 4.4 Laporan patok batas lahan (`laporan-patok`, #331) · 4.5 Mencetak Profil Petani (PDF) (`mencetak-profil-petani`, #343) · 4.6 Mencetak Laporan Pelatihan (`laporan-pelatihan`) · 4.7 Mencetak Laporan Produksi (`laporan-produksi`) · 4.8 Mencetak Laporan Kelompok Tani (`laporan-kelompok-tani`) · 4.9 Menyusun roster anggota per Kelompok Tani (`roster-kelompok-tani`, #257) · 4.10 Memperbarui angka dashboard (`memperbarui-dashboard`) · 4.11 Membuat & menelusuri Snapshot BMP (`snapshot-bmp`, #257) |
