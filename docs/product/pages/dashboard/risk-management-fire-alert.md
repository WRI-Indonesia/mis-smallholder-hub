...
| Server action / data | `getFireBoundaries()` (access-context via `farmerGroupAccessFilter`), `getAdminBoundaries()` (sengaja tanpa scope — garis referensi publik & scope cetak per distrik), `getRiauOutline()` (#280 — outline ter-union untuk memangkas; gagal → `null`, klien jatuh ke poligon kabupaten), fetch klien `GET /api/map-hotspot` (guard `map-parcel` **atau** `dashboard-risk-fire` VIEW) — `?dayRange=` (live) **atau** `?month=YYYY-MM` (#365, eksklusif; keduanya → 400) |
...
| Saringan se-Riau | Logika klien | Hotspot bbox FIRMS dipangkas ke **outline Provinsi Riau ter-union** (`filterPointsWithinAreas`) — bbox persegi ikut memuat Malaysia/Sumbar/Jambi. Sampai #280 pemangkasnya 12 poligon kabupaten yang disederhanakan **sendiri-sendiri**, sehingga batas bersama tak berimpit dan menyisakan celah ±111 m: **9,4 km² wilayah Riau di luar semua poligon**, titik di sana hilang tanpa jejak (479/479 titik uji dari PostGIS terbukti terbuang). Union melarutkan batas dalam **sebelum** simplify, jadi celahnya tak pernah terbentuk |
...
| Sumber boundary | DB | `FarmerGroupBoundary` (seed `scripts/seed/seed-boundary-lembaga.ts`) & `AdministrativeBoundary` (seed `seed-batas-administrasi.ts`; kolom cache `geojson` tersimplifikasi 0,001° **per kabupaten** = untuk render, kolom `geom` resolusi penuh = sumber kebenaran spasial yang dipakai `getRiauOutline()`) |
