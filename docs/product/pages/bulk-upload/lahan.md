...
| Target field (20) | `ID Lahan`*, `ID Petani`*, `Jenis Surat Tanah`, `Nomor Surat`, `Nama tertera di Surat`, `Luas tertera di Surat (ha)`, `Nomor STDB`, `UL Parcel Code (parcel_code)`, `Nama Kelompok Tani` (→ `LandParcel.subGroupLv2`, **isi hanya bila kosong**, update in-place tanpa revisi; pratinjau menandai "(sudah ada)"), **Sepadan U/T/S/B** (#326 — sel terisi menimpa, kosong dibiarkan; sel teks bebas lewat `cleanFreeTextCell`, "Tidak ada" sah), **Blok** (#328 — isi hanya bila kosong, pola KT; Lembaga plasma memakai Blok, swadaya memakai KT), **Status NKT / Kategori NKT / Luas NKT Area (ha) / Panjang NKT (m) / Tanggal Asesmen NKT / Asesor-Sumber NKT** (#328 — status selalu ditimpa = asesmen terbaru menang, field lain hanya bila terisi; alias mengikuti Lampiran HJP: `Luas NKT Area (ha)`, `LENGTH`) — auto-match `PARCEL_DETAIL_AUTO_MATCH_RULES` (`src/lib/land-parcel-detail-import.ts`), termasuk header DBF terpotong `parcel_cod`; tanpa alias satu huruf |
| Bawaan NKT per berkas (#328) | Panel di kartu 2 (sebelum Validasi): status bawaan (— / termasuk / terdampak / tidak), kategori bawaan (checkbox NKT 1–6), tanggal & asesor/sumber bawaan. Untuk daftar "petak terdampak NKT" hasil asesmen yang hanya memuat ID + luas: bila status bawaan dipilih, **semua baris valid** mendapat NKT; sel di berkas selalu menang. Diterapkan di klien (`validateParcelDetailRows(..., nktDefaults)`), server tak perlu tahu |
| Template NKT (#328) | Tombol kedua **Template NKT** di kartu 1: Nama · ID Petani · ID Lahan · Kelompok Tani · Blok · Luas NKT Area (ha) · Panjang (m) · Status/Kategori/Tanggal/Asesor NKT (boleh kosong → bawaan). Importer yang sama; kolom lain yang tak ada = tidak disentuh |
...
| Semantik simpan | **Upsert**: dokumen kunci `(parcelUid, type, number)` aktif → update; STDB unik per petani lewat **dua partial index** sejak #306 (`uniq_land_stdb_farmer_number` untuk baris aktif bernomor, `uniq_land_stdb_farmer_open` untuk satu berkas terbuka per petani) — bukan lagi `@@unique(farmerId, number)`; tautan unik `(parcelUid, stdbId)`; UL Parcel Code unik `(parcelUid, source, code)` dengan `source` = pemeta berkas — sejak 2026-09-23 kode yang **aktif di lahan lain tidak dipindah dan tidak dilewati**: lahan ini mendapat record sendiri, dihitung `externalIdsShared` di toast ("juga dipakai lahan lain — cek silang"); kode sama untuk >1 lahan di satu berkas juga tidak lagi ditolak validator; kode sama dari pemeta berbeda hidup berdampingan |
...
| Objek | Keterangan |
|---|---|
| Kolom | `ID Lahan`* · `ID Petani` (wajib hanya bila ID Lahan dipakai > 1 petani — dihitung **global**, paritas server) · **`Kode Patok`** (#331: `HJP-PTK-000123` → patok fisik itu yang diperbarui; bila belum tertaut ke lahan ini → ditautkan; kode tak dikenal → baris ditolak) · `No Patok` · `Lintang`* · `Bujur`* · `Kondisi` · `Jenis` · `Tanggal Pemasangan` · `Dipasang oleh` · `Keterangan`. Auto-match `MARKER_UPLOAD_AUTO_MATCH_RULES` termasuk nama atribut DBF (`parcel_id`, `no_patok`, `lat/lon`, `x/y`, `cond`, `type`, `installed`; **bukan** `no`/`nomor` — kolom nomor baris spreadsheet akan menimpa patok bernomor sama). Untuk shapefile, `Lintang`/`Bujur` dari geometri ditaruh sebagai kunci pertama record sehingga menang atas atribut DBF X/Y; tombol **Unduh Template Excel** |
...
| Aturan baris | Nomor yang **sudah ada** di lahan → koordinat & atribut patok itu diperbarui (`source` → GPS, berlaku juga untuk lahan lain pemakainya); tanpa nomor tetapi ≤ 5 m dari patok yang sudah tertaut ke lahan ini → patok itu diperbarui (unggah ulang idempoten); nomor baru/kosong → patok baru + tautan, **snap ≤ 5 m** ke patok lahan tetangga (patok nonaktif dihidupkan lagi); > 100 m dari batas ditolak server (per baris, tidak membatalkan batch); lahan tanpa poligon ditolak; nomor ganda dalam berkas ditolak; kondisi/jenis menerima label Indonesia, enum, alias lapangan ("tidak ada" = hilang, "belum ada" = belum dipasang — pembersih teks bebas, bukan token kosong STDB); lat/long tertukar (\|lat\| > 12 & \|lon\| ≤ 12) ditandai sebelum kirim, menggantikan pesan "di luar rentang" |
| Simpan | `bulkUpsertLandMarkers` (CREATE `bulk-upload-parcels`, ≤ 20.000 baris) — scope dihitung sekali untuk seluruh batch, satu kueri jarak per lahan (2N titik), **satu transaksi per lahan** (galat DB satu lahan → baris lahan itu ditolak, lahan lain tetap tersimpan); ringkasan baru · diperbarui · ditautkan · ditolak (ID Lahan + nomor + alasan) |
