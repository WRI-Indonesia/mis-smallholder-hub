...
├── Urut No (select, #371): Abjad pemilik · Posisi lahan
...
| "Urut: …" | Select urutan No (#371, owner 2026-09-23) | **Abjad pemilik** (bawaan = urutan server: Lembaga → KT → nama petani → ID Lahan) · **Posisi lahan** (`sortByMapPosition`: baris paling utara dulu, kiri → kanan; satu baris = lahan paling utara tersisa + lahan yang titik tengah bbox-nya masih dalam rentang lintang lahan itu; tanpa geometri di akhir). Satu urutan untuk tabel, peta pratinjau, PDF, dan Excel (nomor peta = kolom No); di sheet per KT/Blok dihitung ulang per sheet. Mode Posisi tercatat di metadata PDF & sheet Ringkasan ("Urutan No") |
