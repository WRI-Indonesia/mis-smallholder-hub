...
│   └── Catatan filter
├── Kartu KPI
│   ├── Gapoktan/KUD (bila ada level Gapoktan)
│   ├── Kelompok Tani
│   ├── Total Petani
│   ├── Total Lahan
│   └── Total Luas
...
├── Roster collapsible
│   ├── Seksi Gapoktan/KUD (bila ada)
│   │   └── Seksi Kelompok Tani
│   │       └── Tabel Petani
│   │           └── Kolom: No, Nama Petani, ID Petani, Jml Lahan, Luas (Ha)
│   └── (tanpa Gapoktan) Seksi Kelompok Tani → Tabel Petani
└── Ekspor
    ├── Excel
    └── PDF
```
...
| Tipe | Roster hierarkis collapsible (Gapoktan/KUD → Kelompok Tani → Petani) |
...
|---|---|---|
| "Laporan Kelompok Tani (Detail)" | Heading | Deskripsi "Roster per Lembaga Petani: rincian Gapoktan/KUD → Kelompok Tani → daftar Petani (turunan data lahan)" |
...
| Kartu KPI | 4–5 kartu | "Gapoktan/KUD" (hanya bila Lembaga punya level Gapoktan), "Kelompok Tani", "Total Petani", "Total Lahan", "Total Luas" |
| "Buka semua" / "Tutup semua" | Tombol | Default semua seksi tertutup |
| Seksi collapsible | Header seksi | Bila ada Gapoktan → satu seksi per Gapoktan/KUD dengan statistik "`<n>` KT · `<n>` Petani · `<n>` Lahan · `<n>` Ha"; bila tidak → satu seksi per Kelompok Tani dengan "`<n>` Petani · `<n>` Lahan · `<n>` Ha". Nilai kosong ditampilkan "(tidak diketahui)" |
| Empty state | Kartu | "Pilih Lembaga Petani" / saat memuat "Memuat laporan..."; bila tanpa data: "Tidak Ada Data" — "Lembaga Petani ini belum memiliki lahan aktif dengan data Gapoktan/KUD atau Kelompok Tani." |
...
| Format | Keterangan |
|---|---|
| Excel | Sheet "Detail KT", file `Laporan_Kelompok_Tani_Detail_<Lembaga>`; kolom No, Gapoktan/KUD (hanya bila ada), Kelompok Tani, Nama Petani, ID Petani, Jml Lahan, Luas (Ha) + baris Total |
