...
export async function seedCertificationTypes(prisma: PrismaClient) {
  console.log("Seeding certification types...");
  const filePath = path.resolve(__dirname, "data/certification-types.csv");
  ...
  console.log(`Seeded ${data.length} certification types.`);
}
